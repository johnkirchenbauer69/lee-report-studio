...
export function CanvasElement(props: Props) {
  ...
  if (element.type === 'text') { const raw = mode === 'data' && element.binding ? formatValue(getByContextPath(data, element.binding.path, element.bindingContext), element.binding) : element.text;
    ...
    content = <table className={`report-table table-${element.variant??'default'}`}><colgroup>{element.columns.map(c=><col key={c.key} style={c.width?{width:`${c.width}%`}:undefined}/>)}</colgroup><thead><tr>{element.columns.map(c => <th key={c.key} style={{textAlign:c.align}}>{c.label}</th>)}</tr></thead><tbody>{arr.length===0?<tr className="row-empty"><td colSpan={element.columns.length}>{element.emptyMessage??'Data unavailable from selected source'}</td></tr>:arr.map((row, i) => <tr key={i} className={element.rowKindPath?`row-${String(getByPath(row,element.rowKindPath))}`:undefined}>{element.columns.map(c => <td key={c.key} style={{textAlign:c.align}}>{formatValue(getByPath(row, c.path), { path: c.path, format: c.format, decimals: c.decimals??1 })}</td>)}</tr>)}</tbody></table>;
