...
export function CanvasElement(props: Props) {
  ...
  if (element.type === 'text') { const raw = mode === 'data' && element.binding ? formatValue(getByPath(data, element.binding.path), element.binding) : element.text;
    ...
    content = src ? <img src={src} alt={element.name} style={{ width: '100%', height: '100%', objectFit, pointerEvents: 'none' }} /> : <div className="image-placeholder">Image</div>;
  ...
  return <div className={`canvas-element ${selected ? 'is-selected' : ''}`} style={style} onPointerDown={startDrag}
    ...
    {content}{selected && <div className="selection-outline" />}
