...
`POST /api/render/pdf` creates a transient render job. The Chromium renderer loads the requested fixed print route, installs the template's checksum-pinned managed faces, waits for every font and image, applies print CSS, and writes all visible pages in document order. `GET /api/render-jobs/:id` returns state and the completed PDF when ready.
...
`src/services/pdfExport.ts` retains the `pdf-lib` compositor for offline/local fallback. It renders schema primitives directly and supports image crop plus centered basic rotation geometry. It is not the preferred fidelity path because browser text/layout and SVG behavior are better preserved by Chromium. It explicitly refuses managed-font reports and rotated tables/charts instead of silently substituting or dropping transforms.
...
- unavailable fonts or checksum-mismatched managed font references
